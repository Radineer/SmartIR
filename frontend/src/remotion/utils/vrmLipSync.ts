...
/**
 * 音声サンプルデータからRMS（二乗平均平方根）を計算
 * Remotionの getAudioData() で取得した channelWaveforms を入力とする
 */
...
/**
 * 音声波形データからフレームごとのリップシンク状態を計算
 *
 * @param frame - 現在のフレーム番号
 * @param channelSamples - 音声のチャンネルサンプルデータ (Float32Array)
 * @param sampleRate - サンプルレート (e.g. 44100)
 * @param fps - 動画のフレームレート
 * @param text - テキスト（母音推定に使用、オプション）
 * @param textStartFrame - テキスト開始フレーム
 * @param textDurationFrames - テキスト継続フレーム数
 * @returns リップシンク状態
 */
export function calculateLipSyncFromAudio(
  frame: number,
  ...
  // RMS値を計算（音量の指標）
  const rms = calculateRMS(channelSamples, startSample, Math.ceil(samplesPerFrame));

  // RMS → 口の開き度合い（0-1にマッピング）
  // 一般的な音声のRMS範囲 0.0-0.3 を 0-1 にマッピング
  const normalizedIntensity = Math.min(rms / 0.2, 1.0);
  ...
  // 母音の決定: テキストがある場合はテキストベース、なければ音声強度ベース
  let vowel: Vowel;
  if (text && textStartFrame !== undefined && textDurationFrames) {
    const textState = calculateLipSyncFromText(frame, text, textStartFrame, textDurationFrames, fps);
    vowel = textState.vowel;
  } else {
    // 音声のピーク周波数から母音を推定（簡易版）
    // 高RMS+高周波成分 → "i"/"e"、低RMS → "u"/"o"、中間 → "a"
    const vowels: Vowel[] = ["a", "i", "u", "e", "o"];
    const speed = 3 + normalizedIntensity * 4;
    const vowelIndex = Math.floor(frame / speed) % vowels.length;
    vowel = vowels[vowelIndex];
  }

  // 重み: RMSベースの滑らかな口の開き具合
  const weight = Math.min(normalizedIntensity * 1.5, 1.0);
