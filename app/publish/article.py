...
class ArticleGenerator:
    ...
    def generate_analysis_article(
        ...
        title = self._generate_title(company, analysis, label, emoji)
        ...
        parts.append(f"<h2>{company.name}の決算を読み解く</h2>")
        parts.append(
            f"<p>{company.name}（{company.ticker_code}）の最新決算が発表されました。"
            f"全体の評価は<strong>{label}</strong>。"
            f"この決算が意味するものを、数字の裏側から読み解いていきます。</p>"
        )
        parts.append("<hr>")

        # Summary section with context
        if summary_short:
            parts.append("<h2>決算の全体像</h2>")
            parts.append(f"<p>{summary_short}</p>")
            parts.append("<hr>")

        # Key points with framing
        if key_points:
            parts.append("<h2>注目すべき3つのポイント</h2>")
        ...
        if free:
            ...
            parts.append("<hr>")
            parts.append(f"<p>{BRAND} - 決算分析サービス</p>")
            price = 0
        else:
            # Paid section
            parts.append("<pay>")
    ...
    def generate_daily_summary(
        ...
        title = f"【決算まとめ】{date_str} 発表{n}社の決算分析 - {BRAND}"

        parts = []
        parts.append(f"<h2>{date_str} 決算発表まとめ</h2>")
        parts.append(
            f"<p>本日{n}社の決算が発表されました。分析結果をお届けします。</p>"
        )
    ...
    def generate_breaking_article(
        ...
        parts = [
            ...
            "<p>分析を準備中です。詳細な分析結果は後ほど公開予定です。</p>",
            "<hr>",
            f"<p>{BRAND} - 決算分析サービス</p>",
        ]
        return ArticleContent(
            title=title,
            body_html="\n".join(parts),
    ...
    def generate_industry_comparison(
        ...
        parts = [
            ...
            f"<p>{sector}セクターの{n}社の決算を独自スコアリングで分析・比較しました。</p>",
        ]
        for doc, analysis, company in analyses:
            emoji = _sentiment_emoji(analysis)
            label = _sentiment_label(analysis)
    ...
    def generate_earnings_calendar(
        ...
        parts.append("</ul>")
        parts.append("<p>決算発表後、分析記事を順次公開予定です。</p>")
        parts.append("<hr>")
        parts.append(f"<p>{BRAND} - 決算分析サービス</p>")
    ...
    def _generate_title(self, company, analysis, label, emoji):
        """Generate diverse, engaging titles based on analysis content."""
        import random
        
        name = company.name
        code = company.ticker_code
        key_points = analysis.key_points or []
        summary = analysis.summary or ""
        pos = analysis.sentiment_positive or 0
        neg = analysis.sentiment_negative or 0
        
        # Extract a key metric or point for the title
        key_fact = key_points[0][:30] if key_points else ""
        
        patterns = []
        
        if pos >= 0.7:
            patterns = [
                f"{name}、好決算の裏側｜数字が語る成長の構造",
                f"{name}({code})が示す成長シナリオ｜決算から読み解く",
                f"{name}の決算が示す3つの成長ドライバー",
                f"なぜ{name}は好調なのか？決算から見える構造的強さ",
                f"{name}、過去最高益の真相｜投資家が見落としている点",
            ]
        elif neg >= 0.7:
            patterns = [
                f"{name}、厳しい決算の「本当の意味」を読む",
                f"{name}({code})の決算から読む再建シナリオ",
                f"{name}の減益は「生みの苦しみ」か｜構造を深掘り",
                f"数字だけでは見えない{name}の戦略転換",
                f"{name}、減益決算でも注目すべき3つの布石",
            ]
        else:
            patterns = [
                f"{name}({code})決算を徹底分析｜見えてきた課題と機会",
                f"{name}の決算から読む、注目すべき3つの変化",
                f"{name}の現在地｜決算から見える次の一手",
                f"{name}の決算分析｜数字の裏にある経営判断を読む",
                f"{name}({code})の決算、市場が見落としていること",
            ]
        
        return random.choice(patterns)

    def _generate_hashtags(self, company: Company) -> list[str]:
        tags = ["決算", "IR分析", "決算分析", BRAND]
        if company.name:
