...
class AIArticleGenerator:
    ...
    def _call_claude(self, system: str, user_content: str) -> dict | None:
        """Call Claude API with tool_use, falling back to Claude CLI on failure."""
        ...
        # Fallback: Claude CLI (Claude Max via local proxy)
        log.info("Attempting Claude CLI fallback...")
        return self._call_claude_cli(system, user_content)

    def _call_claude_cli(self, system: str, user_content: str) -> dict | None:
        """Call Claude via CLI as fallback (uses Claude Max subscription)."""
        import json
        import subprocess

        prompt = (
            f"{system}\n\n{user_content}\n\n"
            "上記の内容で記事を生成してください。以下のJSON形式で返してください:\n"
            "{\n"
            '  "title": "記事タイトル",\n'
            '  "free_section": "無料公開部分のHTML",\n'
            '  "paid_section": "有料部分のHTML（無料記事なら空文字）",\n'
            '  "iris_comment": "一言コメント",\n'
            '  "hashtags": ["タグ1", "タグ2"]\n'
            "}\nJSONのみを返してください。"
        )
        ...
        try:
            result = subprocess.run(
                ...
                env={
                    **dict(__import__("os").environ),
                    "PATH": f"/Users/apple/.nvm/versions/node/v22.22.2/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin",
                },
            )
            ...
            if start >= 0 and end > start:
                ...
                log.info("Claude CLI fallback succeeded: %s", data.get("title", "")[:50])
                return data

            log.error("Claude CLI: no valid JSON in response")
            return None
        except Exception as e:
            log.error("Claude CLI fallback failed: %s", e)
            return None
    ...
    def generate_breaking_article(
        ...
        user_content = (
            ...
            f"この記事は無料公開です。paid_sectionは空文字列にしてください。\n"
            f"分析はまだ完了していないため、書類の基本情報のみで構成してください。\n"
            f"「詳細な分析結果は後ほど公開予定です」と言及してください。\n\n"
        ...
        body = (
            ...
            f"<p>分析を準備中です。詳細な分析結果は後ほど公開予定です。</p>"
            f"{DISCLAIMER}"
        )
        return ArticleContent(
            title=title,
