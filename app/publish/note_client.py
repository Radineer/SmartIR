...
class NoteClient:
    ...
    async def create_and_publish(
        ...
        # Duplicate check: skip if article with same title already published
        try:
            import httpx as _httpx
            _dup_client = _httpx.Client(headers={"User-Agent": "Mozilla/5.0"})
            for _pg in range(1, 5):
                _r = _dup_client.get(f"https://note.com/api/v2/creators/smartir/contents?kind=note&page={_pg}&per=20")
                _existing = _r.json().get("data", {}).get("contents", [])
                if any(n.get("name") == title for n in _existing):
                    log.warning("Duplicate detected, skipping: %s", title)
                    return {"note_url": "", "skipped": True, "reason": "duplicate_title"}
                if not _existing:
                    break
        except Exception as e:
            log.warning("Duplicate check failed (proceeding): %s", e)
