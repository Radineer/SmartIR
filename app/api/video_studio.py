...
# TTS Engine: "voicevox" or "aivisspeech"
...
class ScriptSegment(BaseModel):
    ...
    speaker_id: int = Field(default=DEFAULT_SPEAKER_ID, description="話者ID")
...
class ScriptGenerateRequest(BaseModel):
    ...
    speaker_id: int = Field(default=DEFAULT_SPEAKER_ID, description="デフォルト話者ID")
...
async def generate_audio(request: AudioGenerateRequest):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            for segment in request.segments:
                ...
                for attempt in range(retry_count):
                    try:
                        # TTS audio_query (VOICEVOX/AivisSpeech compatible API)
                ...
                if wav_data is None:
                    ...
                    raise HTTPException(
                        status_code=502,
                        detail=f"TTS ({TTS_ENGINE}) failed after {retry_count} attempts: {last_error}"
                    )
        ...
        tts_host = get_tts_host()
        logger.error(f"Cannot connect to TTS engine ({TTS_ENGINE}) at {tts_host}")
        raise HTTPException(
            status_code=503,
            detail=f"Cannot connect to TTS engine ({TTS_ENGINE}). Please ensure it is running at {tts_host}"
        )
    except httpx.TimeoutException:
        logger.error(f"TTS ({TTS_ENGINE}) request timed out")
        raise HTTPException(
            status_code=504,
            detail=f"TTS ({TTS_ENGINE}) request timed out"
        )
...
async def studio_health_check():
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            response = await client.get(f"{tts_host}/version")
    ...
    try:
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                # TTS接続確認
                try:
                    await client.get(f"{tts_host}/version")
                except Exception:
                    tts_available = False
                    logger.warning(f"TTS ({TTS_ENGINE}) not available, skipping audio generation")
